...
def createModels(args):
    ...
    for i in range(args.nmodel):
        ...
        if args.model[i] == 'PointNet':
            ...
            from models.NN_MultiG import NN
...
def test(device, model, test_loader, criterion = 'MSE'):
    ...
    for dataF in test_loader:
        ...
        for data_clone in dataF:
            ...
            loss_surf_var = loss_criterion(out[:,0:3], targets[:,0:3] ).mean(dim = 0) ## loss on selectd channels
...
def main():
    models_select = [ 'MLP', ] ## extendable to train multiple models in sequential 
    args = SimpleNamespace()
    args.model=[]
    args.task=[]
    for i in range(len(models_select)):
        args.model.append( models_select[i] )  ## The model you want to train, choose between MLP, PointNet and etc.
    ...
    dataStore_path = "dataset_from_03b_script/training/" ## Note training folder includes val_batch_xx dataset too :: see README
    ...
    base_lr = hparams['lr']
    optimizer = torch.optim.AdamW(model.parameters(), lr = hparams['lr'], weight_decay = weight_decay)
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.1, patience=3, min_lr=1e-7, verbose=True) 
    ...
    train(DEVICE, model, train_loader, val_loader, optimizer, 
          lr_scheduler, log_path, hparams, base_lr, 
          warmup_epochs, criterion='MSE', epochs=hparams["nb_epochs"])
